...
local gtable = require("patchbay.gears.table")
local gsort = require("patchbay.gears.sort")
local object = require("patchbay.object")
local protected_call = require("patchbay.utils").protected_call
...
function matcher:_match(o, rule, matcher_f)
    ...
    for field, value in pairs(rule) do
        ...
        if pm then
            return pm(o, value, field, matcher_f)
...
function matcher:matches_rule(o, entry)
    ...
    if match then
        self:emit_signal("rule::match", o, entry)
    end
...
function matcher:matching_rules(o, rules)
    ...
    if not rules then
        print("This matcher has no rule source")
        return result
    end
...
function matcher:add_matching_function(name, callback, depends_on, precede)
    ...
    if err then
        print("Failed to add the rule source: "..err)
        return false
    end
